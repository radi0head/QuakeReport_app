...
//
        ...
        public static List<EarthquakeItemClass> fetchData(String requestURL){
        ...
        private static String makeHttpRequest(URL requestURL) throws IOException{
            ...
            try{
                ...
                Log.e(LOG_TAG,"Problem retrieving the earthquake JSON response results.",e);
            }finally {
        ...
        /**
         * Return a list of {@link EarthquakeItemClass} objects that has been built up from
         * parsing a JSON response.
         */
        private static List<EarthquakeItemClass> extractEarthquakes(String jsonResponse) {

            // Create an empty ArrayList that we can start adding earthquakes to
            List<EarthquakeItemClass> earthquakes = new ArrayList<>();
            ...
            try {
                ...
                JSONArray earthquakeDetails=root.getJSONArray("features");
                for(int i=0;i<earthquakeDetails.length();i++){
                    JSONObject earthquakeItem=(JSONObject) earthquakeDetails.get(i);
                    JSONObject earthquakeProperties=earthquakeItem.getJSONObject("properties");
                    earthquakes.add(new EarthquakeItemClass(earthquakeProperties.getDouble("mag"),earthquakeProperties.getString("place"),earthquakeProperties.getLong("time"),earthquakeProperties.getString("url")));
                }
